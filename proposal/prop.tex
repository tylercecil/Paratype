...
function \texttt{bar} is declared as accepting and returning a \texttt{T} type.
This introduces a partial context due to \texttt{T} being a type variable. The
parameter type for \texttt{bar} is provided by its parent function
\texttt{foo}. This also completes the return type of \texttt{bar}.
...
Consider the example shown in listing \ref{lst:cbychild}. Function \texttt{foo}
is defined as returning an \texttt{R} type. The function definition contains
the result of calling the \texttt{bar} function as the return value. The
\texttt{bar} function is defined as having an \texttt{int} return type. Since
\texttt{foo} returns the value of calling \texttt{bar} it also has a return
type of \texttt{int}.
...
contexts for each other. One example of this is shown in Listing
\ref{lst:cbyboth}. In this example function \texttt{foo} is providing the type
of the second parameter for function \texttt{barbar}. Function \texttt{barbar}
also provides the type of the first parameter for function \texttt{foo}. Notice
that function \texttt{barbar} is defined as returning a type variable
\texttt{R}. Since \texttt{foo} returns an int type and its return value is a
call to \texttt{barbar}, this resolves the return type for \texttt{barbar}. The
evaluation of these partial contexts is a non-trivial task.
...
the result of type errors. An example of this is shown in Listing
\ref{lst:partial}. The \texttt{bar} function accepts a type variable \texttt{b}
as an input parameter. The function \texttt{foo} provides a context for
\texttt{bar} in which \texttt{b} is of type \texttt{int}. At this point
\texttt{bar} now has a fully evaluated context, but \texttt{baz} can also
complete the partial context with \texttt{b} resolving to type \texttt{float}.
In this situation these are both valid and should not be considered a type
error.
...
listing \ref{lst:unhalting}: \texttt{bar} accepts a type variable as a
parameter. The context provided by its calling function \texttt{foo} resolves
the parameter as being the local variable \texttt{b}. This is problematic due
to \texttt{b} also being a type variable. In this example there is not enough
information to resolve a full context for \texttt{bar}.
...
propagate up call stack. In the example shown in listing \ref{lst:errors}, all
parent functions of \texttt{bar} need to have \texttt{weirdError} in their
fully resolved contexts.
