...
\begin{lstlisting}[caption=Unhalting context resolution,style=paratype]
func foo(T b) int
    = bar(b)
...
(especially if we want to keep communication overhead down)! Each node
...
concurrently; meeting half-way, we can check for type errors. This
...
graph width, number of functions, invalid versus valid code, etc\ldots
...
execution should complete. More subtly, however, are problems that
...
the call tree, do we wait to be signaled, etc\ldots These are all
