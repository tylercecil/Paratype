...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Function{UpdateTypevar}{$C_g, V, C_f, W$}
			...
			\If{$\mathcal{T}_g[ V ] \neq$ incomplete and
				$\mathcal{T}_f[W] \neq$ incomplete and $\mathcal{T}_g[V]
				\neq \mathcal{T}_f[W]$}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State $V_1 \gets \mathcal{A}_g[g \circ f]$ 
		...
		\EndIf
		\State
		\ForAll{$v \in \mathcal{A}_g[g]$} \Comment{If path contains
