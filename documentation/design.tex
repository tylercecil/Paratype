...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Function{setup}{$f$}
			...
			\State $\mathcal{L}_f \gets ()$ \Comment{empty list}
			...
			typevar names}
			\State $i \gets 1$ \Comment{$i$ is counter for typevariable names}
			\ForAll{parameters of $f$ in order}
				\If{typevar name in code is not key in $V$}
					\State $F_i \gets $ new typevar
					\State $V[$ typevar name in code $] \gets F_i$
					\State $\mathcal{M}_f[F_i] \gets $ explicit type or
					\texttt{incomplete}
					\State $\mathcal{K}_f[F_i] \gets $ type classes if
			...
			\ForAll{child function $g$ of $f$ (func composition: outer to inner)}
				\State $R \gets $ typevar of return type of $g$ due to function
				composition (or $F_0$ for outermost function)
				\State $\mathcal{A}_f[f \circ g] \gets (R)$
				\ForAll{parameters of $g$ in order}
					\If{parameter is explicit type}
						\State $F_i \gets $ new typevar
						\State $\Call{append}{\mathcal{A}_f[f \circ g], F_i}$ \Comment{add new
						typevariable to tuple}
						\State $\mathcal{M}_f[F_i] \gets $ explicit type
...
\begin{algorithm}
	\caption{Runtime for each thread}
	\begin{algorithmic}[1]
...
\subsection{Algorithm Terminology}
	...
	\[
	\mathcal{M}(C_f) \subseteq \{ (v, t) : v \in \mathcal{V}, t \in \mathcal{T}
	\cup \{\epsilon\} \}.
	\]
